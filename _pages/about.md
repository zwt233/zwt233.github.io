...
+ **DCML on Graph**: 图数据广泛存在于现实生活中，如微信里的社交网络，知识图谱以及淘宝推荐场景里的用户商品二部图。图机器学习也即“将机器学习应用于图数据”，有望解决传统深度学习无法处理的关系推理、可解释性等一系列问题。我主要考虑以图神经网络（GNN）为切入点，用DCML的思想来优化图数据（如图特征工程、图结构优化、图数据增强和图异常处理等）。
